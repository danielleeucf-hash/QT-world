const passages = {
  john: {
    ...
    hymn: {
      ...
      url: "https://www.youtube.com/results?search_query=%EC%97%AC%ED%98%B8%EC%99%80%EB%8A%94+%EB%82%98%EC%9D%98+%EB%AA%A9%EC%9E%90%EB%8B%88+%EB%82%98%EB%AC%B4%EC%97%94",
      embed: "https://www.youtube.com/embed?listType=search&list=%EC%97%AC%ED%98%B8%EC%99%80%EB%8A%94%20%EB%82%98%EC%9D%98%20%EB%AA%A9%EC%9E%90%EB%8B%88%20%EB%82%98%EB%AC%B4%EC%97%94"
    },
    verses: [
  ...
  matthew: {
    ...
    hymn: {
      ...
      url: "https://www.youtube.com/results?search_query=%EB%A8%BC%EC%A0%80+%EA%B7%B8+%EB%82%98%EB%9D%BC%EC%99%80+%EC%9D%98%EB%A5%BC+%EA%B5%AC%ED%95%98%EB%9D%BC+%EC%98%88%EC%88%98%EC%A0%84%EB%8F%84%EB%8B%A8",
      embed: "https://www.youtube.com/embed?listType=search&list=%EB%A8%BC%EC%A0%80%20%EA%B7%B8%20%EB%82%98%EB%9D%BC%EC%99%80%20%EC%9D%98%EB%A5%BC%20%EA%B5%AC%ED%95%98%EB%9D%BC%20%EC%98%88%EC%88%98%EC%A0%84%EB%8F%84%EB%8B%A8"
    },
    verses: [
  ...
  romans: {
    ...
    hymn: {
      ...
      url: "https://www.youtube.com/watch?v=Y4B_8FVS4SE",
      embed: "https://www.youtube.com/embed/Y4B_8FVS4SE"
    },
    verses: [
...
function createSummary(note) {
  ...
  return note.meditation.length > 150 ? `${note.meditation.slice(0, 150)}...` : note.meditation;
}
...
function saveNote(event) {
  ...
  const note = {
    ...
    meditation: data.get("meditation").trim()
  };

  const hasContent = [note.title, note.meditation].some(Boolean);
  if (!hasContent) {
...
function getBrowserVoice() {
  ...
  return speechSynthesis.getVoices().find((voice) => voice.lang === "ko-KR")
    || speechSynthesis.getVoices().find((voice) => voice.lang.toLowerCase().startsWith("ko"))
    || null;
}
